...
function chartBadge(ctx,chart,level,x,y){const[bg,fg]=CHART_STYLE[chart]||["#eef1f7",INK];rounded(ctx,x,y,66,23,5,bg,null,0);text(ctx,`${chart} ${level}`,x+33,y+12,12,900,fg,"center");}
async function drawScoreTile(ctx,row,x,y,w,h){
 rounded(ctx,x,y,w,h,9,PANEL,LINE,2);
 const bannerW=142,bannerH=h-10,medalSize=Math.min(38,h-10);
 await drawBanner(ctx,row,x+5,y+5,bannerW,bannerH);
 await drawMedal(ctx,row,x+153,y+(h-medalSize)/2,medalSize);
 chartBadge(ctx,row.chart,row.level,x+198,y+h-28);
 text(ctx,`スコア ${String(Number(row.version_score||0)).padStart(5,"0")}`,x+w-7,y+15,11,800,INK,"right");
 text(ctx,`ポックラ ${songPopClass(row).toFixed(2)}`,x+w-7,y+h-13,11,900,INK,"right");
}
function sectionTitle(ctx,label,count,x,y,w){text(ctx,label,x,y+15,19,900,INK);text(ctx,`${count}曲`,x+w,y+15,13,800,MUTED,"right");ctx.fillStyle=LINE;ctx.fillRect(x,y+31,w,2);}
export async function sharePopClassImage(rows,username){
 ...
 await preload(all);
 const width=900,pad=18,gap=7,tileW=(width-pad*2-gap)/2,tileH=52,curRows=Math.ceil(current.length/2),otherRows=Math.ceil(other.length/2);
 const height=84+40+curRows*(tileH+gap)+40+otherRows*(tileH+gap)+32;
 const[c,ctx]=canvas(width,height);header(ctx,"ポックラ対象一覧",`TOTAL ${total.toFixed(2)}`,username,width);
 let y=91;sectionTitle(ctx,"今作 TOP 20",current.length,pad,y,width-pad*2);y+=36;
 for(let i=0;i<current.length;i++){const col=i%2,row=Math.floor(i/2);await drawScoreTile(ctx,current[i],pad+col*(tileW+gap),y+row*(tileH+gap),tileW,tileH);}
 y+=curRows*(tileH+gap)+1;sectionTitle(ctx,"その他 TOP 40",other.length,pad,y,width-pad*2);y+=36;
 for(let i=0;i<other.length;i++){const col=i%2,row=Math.floor(i/2);await drawScoreTile(ctx,other[i],pad+col*(tileW+gap),y+row*(tileH+gap),tileW,tileH);}
...
async function drawMedalSummary(ctx,rows,x,y,w,h){
 ...
 for(let i=0;i<items.length;i++){
  const item=items[i],cx=x+i*cellW,center=cx+cellW/2;
  if(item.type==="dash")text(ctx,"－",center,y+19,19,800,MUTED,"center");
  else if(item.type==="no-play"){text(ctx,"NO",center,y+14,8,800,MUTED,"center");text(ctx,"PLAY",center,y+23,8,800,MUTED,"center");}
  else{const img=await loadImage(item.url);if(img)contain(ctx,img,center-15,y+5,30,30);}
  text(ctx,item.count.toLocaleString("ja-JP"),center,y+h-13,12,900,INK,"center");
 }
}
async function drawLevelTile(ctx,row,x,y,w,h){
 rounded(ctx,x,y,w,h,7,PANEL,LINE,2);
 const medalSize=Math.min(38,h-8),medalArea=46;
 await drawBanner(ctx,row,x+5,y+5,w-medalArea-10,h-10);
 await drawMedal(ctx,row,x+w-medalArea+(medalArea-medalSize)/2,y+(h-medalSize)/2,medalSize);
}
export async function shareLevelMedalImage(rows,level,username){
 ...
 const summaryY=91,summaryH=54,gridY=153,height=gridY+rowCount*(tileH+gap)+31;
