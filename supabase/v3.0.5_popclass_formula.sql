...
language sql stable security definer set search_path=public as $$
  with raw_values as(
    select us.user_id,us.updated_at,us.medal_code,coalesce(us.version_score,0) version_score,
      ...
      +case
         when lower(us.medal_code) in('a','perfect') then 6000
         when lower(us.medal_code) in('b','c','d','fc_1_5','fc_6_20','fc_21_plus') then 5000
         when lower(us.medal_code) in('e','f','g','clear_bad_1_5','clear_bad_6_20','clear_bad_21_plus') then 4000
         when lower(us.medal_code) in('k','easy') then 2000
         when lower(us.medal_code)='long_off' then 3000
         else 0
       end x
    ...
    select user_id,updated_at,medal_code,level,is_current,
      ...
      else floor(((x::numeric/(9810-x::numeric/77)-0.97)*100))/100
      end song_pop_class
    from raw_values
  ), ranked as(
