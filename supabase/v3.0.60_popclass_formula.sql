-- v3.0.60 ポックラ計算式
-- 今作スコアが50000未満なら0。
-- 50000以上: (12000*レベル + 今作スコア - 50000 + クリアボーナス) / 3413
-- クリアボーナス: 未クリア=0 / クリア=17000 / FULL COMBO=18000 / PERFECT=20000
  ...
  with raw_values as(
    select us.user_id,us.updated_at,us.medal_code,coalesce(us.version_score,0) version_score,
      ...
      case lower(coalesce(us.current_clear_status,'failed'))
        when 'perfect' then 20000
        when 'full_combo' then 18000
        when 'clear' then 17000
        else 0
      end clear_bonus
    ...
    select user_id,updated_at,medal_code,level,is_current,
      ...
      else floor((((12000*level::numeric + version_score::numeric - 50000 + clear_bonus)/3413))*100)/100
      end song_pop_class
    from raw_values
  ), ranked as(
