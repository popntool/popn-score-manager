...
begin
  ...
  where coalesce(x.score,0)>0 or coalesce(x.version_score,0)>0 or coalesce(nullif(lower(x.medal_code),'none'),'')<>'' or coalesce(nullif(lower(x.rank_code),'none'),'')<>'';
  ...
  with input as materialized (
    ...
    ) where coalesce(score,0)>0 or coalesce(version_score,0)>0 or coalesce(nullif(lower(medal_code),'none'),'')<>'' or coalesce(nullif(lower(rank_code),'none'),'')<>''
  ...
  -- 同期では曲登録依頼を自動作成しない。未登録譜面はunmatched件数として返すだけにする。
