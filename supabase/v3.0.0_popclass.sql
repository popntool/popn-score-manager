      ...
      case when coalesce(us.version_score,0)=0 then 0::numeric else
        floor((
          (10000*(case us.chart when 'LIGHT' then s.light_level when 'NORMAL' then s.normal_level when 'HYPER' then s.hyper_level when 'EX' then s.ex_level end)
           ...
           +case
              when lower(us.medal_code) in('a','b','c','d','perfect','fc_1_5','fc_6_20','fc_21_plus') then 5000
              when lower(us.medal_code) in('e','f','g','k','clear_bad_1_5','clear_bad_6_20','clear_bad_21_plus','easy','long_off') then 3000
              else 0
            end)::numeric/5440
        )*100)/100
      end song_pop_class
