-- v3.0.80 今作クリア状況を拡張
-- 未プレー / 未クリア / イージー / ロングオフ / クリア / FULL COMBO / PERFECT
...
alter table public.user_scores
  ...
  check (current_clear_status in ('unplayed','failed','easy','long_off','clear','full_combo','perfect'));

-- 手動編集では全状態を保存できる。イージー/ロングオフは公式同期からは生成しない。
...
-- 公式同期:
-- VERSIONプレー回数0回は unplayed。
-- failed が来た場合だけ、既存の easy / long_off を保持する。
-- clear / full_combo / perfect / unplayed は公式結果を優先する。
